...
const Navbar = () => {
    const MenuLink = ({url, path}) => {
        return (
            <li className='pb-3 md:pb-0 text-[20px] md:text-xl text-default font-[500] md:font-semibold'>
            <NavLink
            to={`${url}`}
            className=""
            >
    ...
    return (
        <nav className='flex justify-between items-center fixed top-0 left-1/2 bg-white1 -translate-x-1/2 backdrop-blur-lg py-2 px-10 max-w-[1440px] w-full z-50 shadow-primary '>
            ...
            <article className='hidden md:block static bg-trasparent scale-100 '>
                <ul className='flex items-center gap-9 md:gap-11 font-semibold md:text-xl text-default'>
            ...
            {toggel && (
                <ul className='md:hidden absolute active right-8 left-8 top-[calc(100%+40px)] text-center bg-white rounded-md shadow-md p-8'>
                <MenuLink url="" path="Home"/>
                <MenuLink url="buy" path="Buy" />
                <MenuLink url="rent" path="Rent" />
                <MenuLink url="search" path="Search" />
            ...
            <Link to='/auth' className='hidden md:block'>
                <span className='font-semibold flex items-center gap-1'>Login <BsArrowRight/></span>
            </Link>
